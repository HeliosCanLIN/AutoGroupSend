...
def sentMessage():
    if (config['needSentPicture'] == 1):
        ...
        UpLoad_File()
        """
        while (pyautogui.locateOnScreen(config['sentPng'], grayscale=True,region=(1257, 565, 70, 70)) == None):
            time.sleep(1)
            continue
        """
        while (pyautogui.locateOnScreen(config['sentPng'], grayscale=True) == None):
            time.sleep(1)
            continue
...
if __name__ == '__main__':
    ...
    if (config['groupbroadcast'] == 1):
        ...
        try:
            pointX, pointY = pyautogui.locateCenterOnScreen('+.png', grayscale=True)
        except:
            error("找不到置顶的群")
        pyautogui.moveTo(x=pointX, y=pointY+950)
        while(1):
            pyautogui.click()
            time.sleep(1.5)
            if (pyautogui.locateOnScreen('people.png', grayscale=True) == None):
                print("是群")
                if (sentMessage()):
                    pyautogui.moveTo(x=pointX, y=pointY + 950)
                    #pyautogui.scroll(-87 * j - 2)
                    k = 0
                    while (k < j-2):
            ...
            if (im.getpixel((571, 1031)) != (255, 255, 255)):
                exit(0)
    pyautogui.confirm('请把whatsapp打开，将需要发送的消息写在config.yaml内，如需要中断程序，请将鼠标移动到左上角')
    isFirst=1
    ...
    while (i < total):
        ...
        if (pyautogui.locateOnScreen(config['adminPng'], grayscale=True, confidence=0.98, region=(int(956), int(474), int(400), int(70)))!= None or pyautogui.locateOnScreen('adminEn.png', grayscale=True, confidence=0.98,region=(int(956), int(474), int(400), int(70))) != None or pyautogui.locateOnScreen('+86.png', grayscale=True, region=(int(956), int(474), int(400), int(70)))!= None):
            ...
            i = i + 1
            time.sleep(2)
        if (config['needSentMessage'] == 1 and pyautogui.locateOnScreen(config['plusPng'], grayscale=True)!=None):
            print("1")
            if (config['needSentPicture'] == 1):
                try:
                    pointX, pointY = pyautogui.locateCenterOnScreen('fujian.png', grayscale=True, confidence=0.98)
